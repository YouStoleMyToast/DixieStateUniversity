...
{
	int nextTask, i, switched,temp1,temp2;
	...
	if(tcb[readyq.q[0]].state!=S_BLOCKED)
	{
		for(i=0;i<MAX_TASKS;i++)
		{
			if(readyq.q[i]<0)
			{
				readyq.q[i]=readyq.q[0];
				break;
			}
		}
	}
	...
	while(switched>0)
	{
		...
		{
			if(readyq.q[i]>=0 && readyq.q[i-1]<0)
			{
				readyq.q[i-1]=readyq.q[i];
				readyq.q[i]=-1;
				switched++;
			}
		}
	...
	while(switched>0)
	{
		...
		{
			if(tcb[readyq.q[i]].priority<tcb[readyq.q[i+1]].priority)
			{
				temp1=readyq.q[i+1];
				temp2=readyq.q[i];
				readyq.q[i]=temp1;
...
static void exitTask(int taskId)
{
	...
	for(i=0;i<sizeof(semaphoreList);i++)
	{
		...
		{
			if(semaphoreList[i].semq.q[j]==taskId)
			{
				semaphoreList[i].semq.q[j]=-1;
...
temp:	// ?? temporary label
		...
		if(tid>=0)
		{
			...
			for(i=0;i<MAX_TASKS;i++)
			{
				if(s->semq.q[i]<0 && s->semq.q[i+1]>=0)
				{
					s->semq.q[i]=s->semq.q[i+1];
					s->semq.q[i+1]=-1;
				}
			}
			return;
