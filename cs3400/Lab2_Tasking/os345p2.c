...
int P2_project2(int argc, char* argv[])
{
	...
	static char* timerArgv[] = {"TenSeconds", "tics10sec"};
	...
	createTask("TenSeconds1",				// task name
					...
					2,							// task argc
					timerArgv);				// task argument pointers
	...
	createTask("TenSeconds3",				// task name
					timer,			// task
					HIGH_PRIORITY,			// task priority
					2,							// task argc
					timerArgv);				// task argument pointers
	
	createTask("TenSeconds4",				// task name
					timer,			// task
					HIGH_PRIORITY,			// task priority
					2,							// task argc
					timerArgv);				// task argument pointers
	
	createTask("TenSeconds5",				// task name
					timer,			// task
					HIGH_PRIORITY,			// task priority
					2,							// task argc
					timerArgv);				// task argument pointers
	
	createTask("TenSeconds6",				// task name
					timer,			// task
					HIGH_PRIORITY,			// task priority
					2,							// task argc
					timerArgv);				// task argument pointers
	
	createTask("TenSeconds7",				// task name
					timer,			// task
					HIGH_PRIORITY,			// task priority
					2,							// task argc
					timerArgv);				// task argument pointers
	
	createTask("TenSeconds8",				// task name
					timer,			// task
					HIGH_PRIORITY,			// task priority
					2,							// task argc
					timerArgv);				// task argument pointers
	
	createTask("TenSeconds9",				// task name
					timer,			// task
					HIGH_PRIORITY,			// task priority
					2,							// task argc
					timerArgv);				// task argument pointers
...
{
	int tasknumber, temp0,temp1,swapcount=1,i,combined[200][2];
...
// ?? 3) If blocked, indicate which semaphore
	...
	for (i=0; i<100; i++)
	{	
		combined[i][0]=readyqueue[i][0];
		combined[i][1]=readyqueue[i][1];
		combined[100+i][0]=blockedqueue[i][0];
		combined[100+i][1]=blockedqueue[i][1];
	}
	//sort combined queue
	while (swapcount>0)
	{	
		...
		for(i=0;i<199;i++)
		{
			if(combined[i][1]<combined[i+1][1])
			{
				...
				combined[i+1][0]=temp0;
				combined[i+1][1]=temp1;
				swapcount++;
			}
		}
	}
	...
	{	
		if (combined[i][1]>=0)
		{
			tasknumber=combined[i][0];
			printf("\n%4d/%-4d%20s%4d  ", tasknumber, tcb[tasknumber].parent,
		  				tcb[tasknumber].name, tcb[tasknumber].priority);
			if (tcb[tasknumber].signal & mySIGSTOP) my_printf("Paused");
			else if (tcb[tasknumber].state == S_NEW) my_printf("New");
			else if (tcb[tasknumber].state == S_READY) my_printf("Ready");
			else if (tcb[tasknumber].state == S_RUNNING) my_printf("Running");
			else if (tcb[tasknumber].state == S_BLOCKED) my_printf("Blocked    %s",
		  				tcb[tasknumber].event->name);
			else if (tcb[tasknumber].state == S_EXIT) my_printf("Exiting");
			swapTask();
		}
	}
	return 0;
