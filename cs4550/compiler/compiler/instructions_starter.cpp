...
{
	if(mCurrent<MAX_INSTRUCTIONS) { mCode[mCurrent++]=c; }
	else
	{
...
/*
void InstructionsClass::Encode(bool x)
{
	if(mCurrent<(MAX_INSTRUCTIONS-1)) 
	{
		*((bool*)(&( mCode [mCurrent]))) = x;
		mCurrent += 1;
	}
	else
	{
...
void InstructionsClass::Encode(void * p)
{
	...
	if (pointerSize==4){Encode((int)(long long)p);}
	else if(sizeof(p)==8){Encode((long long)p);}
	//else if(pointerSize==1){Encode((bool)p);}
	else{assert(false);}
}
...
/*
void InstructionsClass::PushBool(bool boolean)
{
	cout<<"Pushing bool "<<boolean<<endl;
	Encode(IMMEDIATE_TO_EAX);
	Encode(boolean);
	Encode(PUSH_EAX);
}*/
...
void HelperPrintInteger(void){printf("%i ", gPrintInteger);}
...
// This is called by the generated machine language code.
void HelperPrintEndl(void){cout << endl;}
void InstructionsClass::WriteEndl(){Call((void*)HelperPrintEndl);}
void InstructionsClass::PopPopAddPush()//good
{
	cout<<"PopPopAddPush "<<endl;
	Encode(POP_EBX);
	Encode(POP_EAX);
	Encode(ADD_EAX_EBX1);
	Encode(ADD_EAX_EBX2);
...
void InstructionsClass::PopPopDivPush()//good
{
	cout<<"PopPopDivPush "<<endl;
	Encode(POP_EBX);
	Encode(POP_EAX);
	Encode(CDQ);// Necessary to clear the D register for a 64 bit divide.
	Encode(DIV_EAX_EBX1);
...
void InstructionsClass::PopPopComparePush(unsigned char relational_operator)//good
{
	cout<<"PopPopComparePush "<<endl;
	Encode(POP_EBX);
	Encode(POP_EAX);
	Encode(CMP_EAX_EBX1);
	Encode(CMP_EAX_EBX2);
	...
	Encode(0);
	Encode(PUSH_EAX); // push 1 or 0 //orginally set to zero then overwriten by relational_operator if necisary
}
void InstructionsClass::PopPopLessPush(){PopPopComparePush(JL);}//good
void InstructionsClass::PopPopLessEqualPush(){PopPopComparePush(JLE);}//good
void InstructionsClass::PopPopGreaterPush(){PopPopComparePush(JG);}
void InstructionsClass::PopPopGreaterEqualPush(){PopPopComparePush(JGE);}
void InstructionsClass::PopPopEqualPush(){PopPopComparePush(JE);}
void InstructionsClass::PopPopNotEqualPush(){PopPopComparePush(JNE);}
void InstructionsClass::PopPopAndPush()//good //teachers brute force way of doing it, there may be a better way
{
	Encode(IMMEDIATE_TO_EAX); // load A register with 0
	Encode(0);
...
void InstructionsClass::SetOffset(unsigned char *codeAddress, int offset){*((int*)codeAddress)=offset;}
unsigned char *InstructionsClass::GetAddress(){return &(InstructionsClass::mCode[mCurrent]);}
