...
class Platformer(game.Game):
    ...
    def initialize(self):
        self.themap = map.Map(self.map_filename[self.currentmap], self.tiles, self.screen,
    ...
    def game_logic(self, keys, newkeys):
        if self.changemap==True:
            self.changemap=False
            self.currentmap+=1
            self.initialize()

        if self.theguy.levelchange:
            self.changemap=True

        if self.currentmap==0 and self.theguy.x==self.theguy.startx+10:
            self.theguy.playintro()
