...
class GenerateCode:
    ...
    def getparameters(self,token):
        ...
        endfound=False
        while n<=len(self.tokenlist)-(1+self.i) and endfound==False:
            parenend=self.peek(self.token,n)
            if parenend[1]==')':
                endfound=True
            else:
                n+=1
    ...
    def docode(self,token):
        ...
        if fromdo==True:
            ...
            classname=None
            n=self.foundbyendofline(token,"Identifier")
            classname=self.peek(token,n)
            classname=classname[1]
            print >>self.fout, 'class',classname
            print >>self.fout, 'functionname',functionname
            print>>self.fout,"call",str(classname)+"."+str(functionname),hardcode
            print>>self.fout,"pop temp 0"
        elif fromdo==False:
            print>>self.fout,"pop constant 1"
            print>>self.fout,"pop this 0"
            self.tokennum+=1
            
    def letcode(self,token):
        self.expresioncode(token)
        print >> self.fout,"pop argument 0"

    def foundbyendofline(self,token,item):
        n=1
    ...
    def encode(self):
        while self.tokennum < self.tokenlen:
            token=self.tokenlist[self.tokennum]
            self.i+=1
            self.tokennum+=1
            if token[0]=="Keyword":
                ...
                if token[1]=="function":
                    ...
                    functionname=functionname[1]
                    print>>self.fout,"function",self.classname+"."+functionname,classlocalscount

                if token[1]=="method":
                    methodname=self.peek(token,1)
                    methodname=methodname[1]
                    paramatercount=self.getparameters(self.token)
                    print>>self.fout,"method",self.classname+"."+methodname,paramatercount
                    print>>self.fout,"push argument 0"
                ...
                if token[1]=="constructor":
                    ...
                    print>>self.fout,"push constant",classlocalscount
                    print>>self.fout,"call memory.alloc",argument
                    print>>self.fout,"pop pointer 0"
                ...
                if token[1]=="if":
                    self.expresioncode(token)
                    print>>self.fout,"not"
                    print>>self.fout,"if.goto IF_ELSE_0"
                    
                    self.statementcode(token)
                    print>>self.fout,"goto IF_EXIT_0"
                    print>>self.fout,"label IF_ELSE_0"
                    self.statementcode(token)
                    print>>self.fout,"label IF_EXIT_0"
                ...
                if token[1]=="while":
                    print>>self.fout,"Label WHILE_EXIT_0"
                    self.expresioncode(token)
                    
                    print>>self.fout,"not"
                    
                    print>>self.fout,"if.goto WHILE_EXIT_0"
                    self.statementcode(token)
                    print>>self.fout,"goto WHILE_TEST_0"
                    print>>self.fout,"local WHILE_EXIT_0"
